...
void setup_pointatoms_communicator_receiver(
  ...
  using base_receiver_t =
      ANANSI::PointAtomsCommunicator::MyParentTask; // The base class for the
                                                    // task we are setting up.
  using concrete_receiver_t =
      ANANSI::PointAtomsCommunicator; // The concrete tasks we are setting up.
  ...
  std::shared_ptr<ANANSI::AnansiTask> my_task =
      concrete_task_factory->create_shared_ptr<base_receiver_t>(
          point_atoms_communicator_receiver);
