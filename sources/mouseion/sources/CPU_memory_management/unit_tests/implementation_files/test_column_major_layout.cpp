 ...
 * =====================================================================================
 *
 *       Filename:  test_column_major_layout.cpp
 *
 *    Description:  
 *
...
{
    return std::string("test_column_major_layout.logfile.txt");
}
    ...
    for (auto row_size=min_matrix_row_size; row_size <= max_matrix_row_size; ++row_size)
    {
        for (auto column_size=min_matrix_column_size; column_size <= max_matrix_column_size; ++column_size)
        {
            FixtureArray2d const my_2d_array(row_size,
                                             column_size, 
                                             ArrayLayoutType::column_major,
                                             pairing_function);

            std::tuple<bool,std::string> results = verify_valid_2d_column_major_layout(row_size,
                                                                                       column_size,
...
{
    Array2dPrintSpecs my_print_specs(ArrayLayoutType::column_major,
                                     "Column Major Matrix Errors",
                                     row_size,
                                     column_size);

    std::ofstream logFileOstream;
    logFileOstream.open("test_column_major_layout.logfile.txt",std::ios::app);

    my_print_specs.printHeaders(logFileOstream);

    bool valid_layout = true; 
    const std::string no_error_message("Valid 2d column major layout.");
    ...
    for (auto jp=0; jp < column_size; ++jp)
    {
        for (auto ip=0; ip < row_size; ++ip)
        {
            ...
            if ( delta != 0  )
            {
                ...
                error_message += "Error in column major layout.";
            }
            ++disp;
        }
    }
    ...
    return results;
} // -----  end of method verify_valid_2d_column_major_layout  ----- 
