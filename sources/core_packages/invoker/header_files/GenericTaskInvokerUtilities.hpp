...
public:
  ...
  static void modifyTaskReceiver(std::shared_ptr<ANANSI::AnansiTask> &task,
    ...
    std::shared_ptr<concrete_task_t> p_concrete =
        AnansiTaskUtilities<ANANSI::AnansiTask,
                            concrete_task_t>::asConcreteTask(task);
  ...
  template <typename ConcreteProductsTypeList, RECEIVER::TaskLabel COMMAND_KEY>
  static void verifyConcreteProductInTypeList() {
