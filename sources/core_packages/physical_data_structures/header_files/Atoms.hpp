...
class Atoms {
  public:
    ...
    template <typename T>
    Atoms(T &&value) : valuePtr_(new AtomsModel<T>(std::forward<T>(value))) {
        return;
    }
  ...
  private:
    ...
    template <typename T> class AtomsModel : public AtomsConcept {
      public:
        // ====================  LIFECYCLE
        // =======================================
        AtomsModel() : AtomsConcept(), object_() { return; };

        AtomsModel(const AtomsModel &other)
            : AtomsConcept(other), object_(other.object_) {
            if (this != &other) {
            }
        };
        ...
        explicit AtomsModel(const T &in_value) : object_(in_value) { return; };
