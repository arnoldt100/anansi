...
//! of a key and value where each key is unique. The key and value are
...
public:
  ...
  CommandFiles() : valuePtr_(nullptr) { return; }
  ...
  // ====================  ACCESSORS     =======================================
  CommandFiles clone() const { return CommandFiles(*this); }
...
private:
  ...
  // The model.
  template <typename T> class CommandFilesModel : public CommandFilesConcept {
  public:
    // ====================  LIFECYCLE =======================================
    CommandFilesModel() : CommandFilesConcept(), object_() { return; };

    CommandFilesModel(const CommandFilesModel &other)
        : CommandFilesConcept(other), object_(other.object_) {
      if (this != &other) {
      }
    };
    ...
    explicit CommandFilesModel(const T &in_value) : object_(in_value) {
      return;
    };
  ...
  friend void set_CommandFile_filename(CommandFiles &&command_file,
                                       const CommandFileName filename) {
  ...
  friend void write_file_CommandFile(CommandFiles &command_file,
  ...
  friend std::string
  get_default_null_value_CommandFile(CommandFiles &command_file) {
